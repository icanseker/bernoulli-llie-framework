# An Entropy-Driven and Precision-Scalable Framework for Low-Light Image Enhancement via Analytic Functions Subordinate to Bernoulli Polynomials
...
The proposed method leverages **coefficient bounds derived from bi-univalent Sakaguchi-type analytic functions**, where the subordinating function is expressed in terms of the **classical Bernoulli polynomial family**. These coefficient bounds are used to construct **8-directional 3×3 spatial convolution kernels**, which are applied to very dark images. The parameters governing the coefficient formulas (ν and t) are not fixed — they are **dynamically optimized per image** using a two-phase strategy: a coarse grid search followed by Nelder-Mead simplex refinement, with Shannon entropy as the optimization criterion.

Nine established low-light enhancement algorithms serve as comparison baselines, each representing a fundamentally different algorithmic category. All experiments are conducted on the **LOL (Low-Light) paired dataset**. On LOL-15, the proposed method achieves the **Best NIQE (3.81)** among all 10 methods and **33/90 total metric wins** — the highest win count.
...
3. [Dataset](#dataset)
4. [Proposed Method — Bernoulli](#proposed-method--bernoulli)
...
│   ├── run_bernoulli.m                 # Proposed: Bernoulli polynomial coefficient convolution
...
└── target/                           # Output directory (created on run)
    └── {AlgorithmName}/              # One subfolder per algorithm
        ...
        └── {id}_metrics.txt          # Metrics + optimal (nu, t) for Bernoulli
```
...
cd /path/to/bernoulli-llie-framework
run('main.m')
```
...
All 15 LOL evaluation images are included as defaults in `source/low/` and `source/high/`. Every image in the evaluation set has a mean intensity (I_mean) well below the threshold of 64, so all 15 are within the intended operating range of the proposed Bernoulli method.
...
- The proposed Bernoulli method will only apply convolution-based enhancement to images with **I_mean < 64**. Images above this threshold are logged as SKIPPED for the polynomial method (see below).
...
## Proposed Method — Bernoulli

### Mathematical Formulation

The method derives its enhancement kernel from the **coefficient bounds of a subclass of bi-univalent Sakaguchi-type analytic functions**, where the subordinating structure is built using **classical Bernoulli polynomials**.
...
Given parameters ν > 0 and t ∈ [-1, 1), the Bernoulli polynomial coefficient bounds are:
...
- ν ∈ (0.5, 1.0] — ν > 0.5 strictly: at ν = 0.5, (2ν−1) = 0 making a₂ = 0 (degenerate kernel)
...
- ν ∈ [0.51, 1.00] (ν > 0.5 strictly: at ν=0.5, (2ν−1)=0 makes a₂=0 degenerate)
- t ∈ [-1.0, 0.9]
...
For each (ν, t) pair:
1. Compute Bernoulli coefficients a₂, a₃ — skip if invalid (denominator ≤ 0, non-finite)
...
```
Algorithm: Bernoulli Low-Light Enhancement
...
─────────────────────────────────────────────────────────────────────
STEP 0: RESOLVE OPTIMIZATION PRESET  (inside run_bernoulli.m)
─────────────────────────────────────────────────────────────────────
...
─────────────────────────────────────────────────────────────────────
  ...
  Initialize results ← empty table
    for each nu in [0.51 : nu_step : 1.00]:   // nu_step from preset
      for each t in [-1.0 : t_step : 0.9]:  // t_step from preset

          // Compute Bernoulli coefficient bounds (Theorem 1)
          numer ← 3 · (2·nu − 1)³
          denom ← 2·(1 − t)·((6·nu² − 18·nu + 7)·t + 6·nu² + 6·nu − 4)
...
─────────────────────────────────────────────────────────────────────
  ...
  for k = 1 to n_candidates:   // n_candidates from preset
      ...
      x_opt[1] ← clamp(x_opt[1], 0.51, 1.00)   // nu
...
─────────────────────────────────────────────────────────────────────
  Compute final coefficients from (best_nu, best_t):
      a2, a3 ← bernoulli_coefficients(best_nu, best_t)
...
OUTPUT
─────────────────────────────────────────────────────────────────────
  Save I_enh  → target/Bernoulli/{id}_enhanced.png
  Save metrics → target/Bernoulli/{id}_metrics.txt
...
**Why EnlightenGAN over supervised DL methods:** Supervised methods such as RetinexNet are trained on paired datasets including LOL. Including them as comparisons would give them an unfair structural advantage on the LOL evaluation set. EnlightenGAN is unsupervised — it was not trained on LOL pairs — making the comparison fair. Both the proposed Bernoulli method and EnlightenGAN operate without dataset-specific optimization, enabling a clean comparison of "training-free mathematical method vs. training-free neural network."
...
|-----------|----------|-------------|-----------|
| **Bernoulli (Proposed)** | **Polynomial coefficient convolution** | **Per-image (ν, t) via entropy max.** | **Dynamic** |
...
### metrics.txt Format — Proposed Method (Bernoulli)
...
optimization_preset = 'high_precision';
                             ...
                             % tolerances are both resolved inside run_bernoulli.m.
                             % Options: 'high_precision' | 'balanced' | 'fast'
                             % Change this one line to switch modes.
```

**Preset summary (all grid + Nelder-Mead parameters resolved inside `run_bernoulli.m`):**

| Preset | Phase 1 Grid | Grid Evals | n_cand | NM TolFun/TolX | NM MaxEvals | ~Time/image | Use case |
|--------|-------------|-----------|--------|----------------|-------------|-------------|----------|
| `high_precision` | nu=0.03, t=0.05 (~663 pts) | ~663 | 6 | 1e-6 | 1100 | ~1.32 s | Published results |
| `balanced` | nu=0.05, t=0.08 (~240 pts) | ~240 | 4 | 1e-4 | 400 | ~0.58 s | Development |
| `fast` | nu=0.10, t=0.20 (~50 pts) | ~50 | 2 | 1e-2 | 100 | ~0.19 s | Real-time / live video / live video |
...
The **entire optimization pipeline** is controlled by a single `optimization_preset` string in `main.m`. All Phase 1 grid parameters and Phase 2 Nelder-Mead tolerances are resolved inside `run_bernoulli.m`. No other edits are needed.
...
|--------|---------|--------|---------|--------|-------------|------------|-----------|----------|
| **`high_precision`** | 0.03 | 0.05 | ~663 | 6 | 1e-6 | 1100 | ~1.32 s | Published results |
| **`balanced`** | 0.05 | 0.08 | ~240 | 4 | 1e-4 | 400 | ~0.58 s | Development |
| **`fast`** | 0.10 | 0.20 | ~50 | 2 | 1e-2 | 100 | ~0.19 s | Real-time / live video / live video |
...
The preset is passed as a string to `run_bernoulli`, which resolves all parameters including n_candidates. Note that n_candidates is set to 6/4/2 to ensure adequate coverage of the narrow ν domain [0.51, 1.00].

```matlab
% run_bernoulli.m — preset resolution:
switch lower(preset)
    case 'high_precision'
...
[~, best_nu, best_t, ~] = run_bernoulli(img_thumb, optimization_preset);
