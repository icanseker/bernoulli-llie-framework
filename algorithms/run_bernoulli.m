function [enhanced, best_nu, best_t, total_evals] = run_bernoulli(img_low, preset)
%RUN_BERNOULLI  Enhance a low-light image using Bernoulli polynomial coefficient bounds.
%
%  Paper: "An Entropy-Driven and Precision-Scalable Framework for Low-Light Image Enhancement via Analytic Functions Subordinate to Bernoulli Polynomials"
%
%  ── Architecture ────────────────────────────────────────────────────────────
...
%  time. high_precision uses ~663 grid points, covering the [0.51,1.00] × [-1.0,0.9]
%  parameter space densely. ν > 0.5 strictly: at ν = 0.5, (2ν−1) = 0 making a₂ = 0 (degenerate kernel).
%
%  ── Preset system ────────────────────────────────────────────────────────────
...
%  total_evals - Total entropy evaluations (grid + all Nelder-Mead runs)
    ...
    switch lower(preset)
        case 'high_precision'
            % Finest grid: ~663 points across [0.51,1.00] × [-1.0,0.9]
        ...
        case 'fast'
            ...
            nm_MaxFunEvals = 100;
        otherwise
            error('run_bernoulli:unknownPreset', ...
    ...
    for nu = nu_range
        for t = t_range
            [a2, a3, valid] = bernoulli_coefficients(nu, t);
    ...
    for k = 1:n_cand
        ...
        obj = @(x) fast_neg_entropy(x, img_d, I_edge, @bernoulli_coefficients, ...
...
%% ═══════════════════════════════════════════════════════════════════════════
%  LOCAL FUNCTION: bernoulli_coefficients
%  ═══════════════════════════════════════════════════════════════════════════
function [a2, a3, valid] = bernoulli_coefficients(nu, t)
%BERNOULLI_COEFFICIENTS  Compute Bernoulli polynomial coefficient bounds.
%
%  Formulas (Theorem 1, Bernoulli family):
%
%    a₁ = 1  (fixed by bi-univalent normalization)
%
%    numer = 3·(2ν−1)³
%    denom = 2·(1−t)·((6ν²−18ν+7)·t + 6ν²+6ν−4)
%    a₂ = sqrt(numer / denom)
%
%    a₃ = (2ν−1)/[2(1−t)(t+2)]  +  (2ν−1)²/[4(1−t)²]
%
%  Domain: ν ∈ (0.5, 1.0], t ∈ [−1, 1).
%  ν > 0.5 strictly: at ν = 0.5, (2ν−1) = 0 making a₂ = 0 (degenerate).

    valid = false; a2 = 0; a3 = 0;

    numer = 3 * (2*nu - 1)^3;
    denom = 2 * (1 - t) * ((6*nu^2 - 18*nu + 7)*t + 6*nu^2 + 6*nu - 4);
